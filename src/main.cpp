...
//---------- Deklarasi Fungsi ----------
float readCurrent();
String buildMonitoringJSON(bool addTimestamp);
String getTimestamp();
...
//---------- Input and output pins ----------
const int INPUT_PINS[] = {12, 13, 14, 25, 26, 27, 32, 33};                                 // Pin input
const int INPUT_PINS_PCF8574[] = {0, 1, 2, 3, 4, 5, 6, 7};                                 // Pin input expansion
const int OUTPUT_PINS[] = {4, 15, 16, 17};                                                 // Pin output
const int NUM_INPUTS = sizeof(INPUT_PINS) / sizeof(INPUT_PINS[0]);                         // Sum of input pins
const int NUM_INPUTS_PCF8574 = sizeof(INPUT_PINS_PCF8574) / sizeof(INPUT_PINS_PCF8574[0]); // Sum of input pins
...
bool checkInputChanges()
{
  ...
  for (int i = 0; i < 8; i++)
  {
    uint8_t current = pcf8574.digitalRead(i);
...
void setup()
{
  ...
  // Initialize pin IO Extention
  for (int i = 0; i < NUM_INPUTS_PCF8574; i++)
  {
    pcf8574.digitalWrite(INPUT_PINS_PCF8574[i], INPUT_PULLUP);
  }
